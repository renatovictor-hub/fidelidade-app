...
export default async function handler(req, res) {
  ...
  try {
    ...
    let telefones = [];
    let publico = "Todos los clientes";
    let todos = segmento === "todos";
    ...
    if (!todos) {
      ...
      const usuarios = Object.entries(usersObj).map(([uid, u]) => ({ uid, ...(u || {}) }));

      if (segmento === "cliente") {
        const busca = String(valorSegmento || "").trim();
      ...
      if (segmento === "inativos_dias") {
        const dias = Math.max(1, Number(valorSegmento || 30));
    ...
    const historicoRef = db.ref("push_historico").push();
    await historicoRef.set({
      titulo,
      mensagem: desc,
      segmento,
      publico,
      destinatarios_estimados: todos ? null : telefones.length,
      imagem: imagem || "",
